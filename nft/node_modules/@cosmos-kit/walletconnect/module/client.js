...
export class WCClient {
  constructor(walletInfo) {
    _defineProperty(this, "walletInfo", void 0);
    _defineProperty(this, "signClient", void 0);
    _defineProperty(this, "wcCloudInfo", void 0);
    // info from WalletConnect Cloud Explorer
    _defineProperty(this, "actions", void 0);
    _defineProperty(this, "qrUrl", void 0);
    _defineProperty(this, "appUrl", void 0);
    _defineProperty(this, "pairings", []);
    _defineProperty(this, "sessions", []);
    _defineProperty(this, "emitter", void 0);
    _defineProperty(this, "logger", void 0);
    _defineProperty(this, "options", void 0);
    _defineProperty(this, "relayUrl", void 0);
  ...
  subscribeToEvents() {
    ...
    this.signClient.on('session_update', ({
      ...
      this.logger?.debug('EVENT', 'session_update', {
        topic,
        params
      });
      // if (this.session?.topic != topic) return;
  ...
  restorePairings() {
    ...
    this.pairings = this.signClient.pairing.getAll({
      active: true
    }).filter(p => p.peerMetadata?.name === this.wcName && p.expiry * 1000 > Date.now() + 1000);
  ...
  restoreSessions() {
    ...
    this.sessions = this.signClient.session.getAll().filter(s => s.peer.metadata.name === this.wcName && s.expiry * 1000 > Date.now() + 1000);
  ...
  async getAccount(chainId) {
    const {
      ...
      algo,
      pubkey
    } = (await this._getAccount(chainId))[0];
    return {
      address,
      algo: algo,
  ...
  async signDirect(chainId, signer, signDoc, signOptions) {
    const {
      signed,
      signature
    } = await this._signDirect(chainId, signer, signDoc, signOptions);
    return {
      signed: {
