"use strict";

var _interopRequireDefault = require("@babel/runtime/helpers/interopRequireDefault");
Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.themeList = exports.styleData = exports.semanticTokens = void 0;
var _slicedToArray2 = _interopRequireDefault(require("@babel/runtime/helpers/slicedToArray"));
var _toConsumableArray2 = _interopRequireDefault(require("@babel/runtime/helpers/toConsumableArray"));
var _defineProperty2 = _interopRequireDefault(require("@babel/runtime/helpers/defineProperty"));
var _types = require("../utils/types");
function ownKeys(object, enumerableOnly) { var keys = Object.keys(object); if (Object.getOwnPropertySymbols) { var symbols = Object.getOwnPropertySymbols(object); enumerableOnly && (symbols = symbols.filter(function (sym) { return Object.getOwnPropertyDescriptor(object, sym).enumerable; })), keys.push.apply(keys, symbols); } return keys; }
function _objectSpread(target) { for (var i = 1; i < arguments.length; i++) { var source = null != arguments[i] ? arguments[i] : {}; i % 2 ? ownKeys(Object(source), !0).forEach(function (key) { (0, _defineProperty2["default"])(target, key, source[key]); }) : Object.getOwnPropertyDescriptors ? Object.defineProperties(target, Object.getOwnPropertyDescriptors(source)) : ownKeys(Object(source)).forEach(function (key) { Object.defineProperty(target, key, Object.getOwnPropertyDescriptor(source, key)); }); } return target; }
function groupBy(objectArray, property) {
  return objectArray.reduce(function (acc, obj) {
    var _acc$key;
    var key = obj[property];
    var curGroup = (_acc$key = acc[key]) !== null && _acc$key !== void 0 ? _acc$key : [];
    return _objectSpread(_objectSpread({}, acc), {}, (0, _defineProperty2["default"])({}, key, [].concat((0, _toConsumableArray2["default"])(curGroup), [obj])));
  }, {});
}
function handleData(data) {
  var initialObj = groupBy(data, 'category');
  var val = {};
  for (var _i = 0, _Object$entries = Object.entries(initialObj); _i < _Object$entries.length; _i++) {
    var _Object$entries$_i = (0, _slicedToArray2["default"])(_Object$entries[_i], 2),
      key = _Object$entries$_i[0],
      value = _Object$entries$_i[1];
    var valueObj = value.flatMap(function (_ref) {
      var componentName = _ref.componentName,
        style = _ref.style,
        theme = _ref.theme;
      return theme.flatMap(function (_ref2) {
        var themeName = _ref2.themeName,
          themeValue = _ref2.themeValue;
        var objKey = "".concat(componentName, "-").concat(style, "-").concat(themeName);
        return (0, _defineProperty2["default"])({}, objKey, themeValue);
      });
    });
    val = _objectSpread(_objectSpread({}, val), {}, (0, _defineProperty2["default"])({}, key, Object.assign.apply(Object, [{}].concat((0, _toConsumableArray2["default"])(valueObj)))));
  }
  return val;
}
var themeList = [{
  name: _types.Themes.Light,
  displayColor: '#fff',
  colorMode: _types.Themes.Light
}, {
  name: _types.Themes.Dark,
  displayColor: '#1A202C',
  colorMode: _types.Themes.Dark
}];
exports.themeList = themeList;
var styleData = [
// connect-wallet-button
{
  componentName: 'connect-wallet-button',
  category: 'colors',
  style: 'background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'primary.500'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'primary.400'
  }]
}, {
  componentName: 'connect-wallet-button',
  category: 'colors',
  style: 'hover-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'primary.400'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'primary.500'
  }]
}, {
  componentName: 'connect-wallet-button',
  category: 'colors',
  style: 'active-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'primary.50'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'primary.50'
  }]
}, {
  componentName: 'connect-wallet-button',
  category: 'colors',
  style: 'disabled-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'gray.50'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'gray.700'
  }]
}, {
  componentName: 'connect-wallet-button',
  category: 'colors',
  style: 'text-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'white'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'white'
  }]
}, {
  componentName: 'connect-wallet-button',
  category: 'colors',
  style: 'active-text-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'primary.500'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'primary.400'
  }]
}, {
  componentName: 'connect-wallet-button',
  category: 'colors',
  style: 'active-text-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'primary.500'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'primary.400'
  }]
}, {
  componentName: 'connect-wallet-button',
  category: 'shadows',
  style: 'focus-shadow',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: '0 0 0 2px var(--chakra-colors-primary-200)'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: '0 0 0 2px var(--chakra-colors-primary-200)'
  }]
},
// copy-address-button
{
  componentName: 'copy-address-button',
  category: 'colors',
  style: 'hover-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'blackAlpha.50'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'whiteAlpha.50'
  }]
}, {
  componentName: 'copy-address-button',
  category: 'colors',
  style: 'text-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'gray.500'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'whiteAlpha.600'
  }]
}, {
  componentName: 'copy-address-button',
  category: 'colors',
  style: 'border-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'gray.200'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'whiteAlpha.300'
  }]
},
// install-wallet-button
{
  componentName: 'install-wallet-button',
  category: 'colors',
  style: 'background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'rgba(37, 57, 201, 0.1)'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'rgba(40, 62, 219, 0.15)'
  }]
}, {
  componentName: 'install-wallet-button',
  category: 'colors',
  style: 'border-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'white'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'gray.800'
  }]
}, {
  componentName: 'install-wallet-button',
  category: 'colors',
  style: 'text-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'primary.400'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'primary.100'
  }]
}, {
  componentName: 'install-wallet-button',
  category: 'shadows',
  style: 'shadow',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: '0 0 1px 2px rgba(37, 57, 201, 0.5)'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: '0 0 1px 2px rgba(196, 203, 255, 0.5)'
  }]
},
// change-chain-dropdown
{
  componentName: 'change-chain-dropdown',
  category: 'colors',
  style: 'firefox-scrollbar-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'rgba(0, 0, 0, 0.15) transparent' // thumb-color background-color
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'rgba(255, 255, 255, 0.15) transparent' // thumb-color background-color
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'colors',
  style: 'scrollbar-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'transparent'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'transparent'
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'colors',
  style: 'scrollbar-thumb-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'blackAlpha.200'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'whiteAlpha.200'
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'colors',
  style: 'indicator-text-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'blackAlpha.600'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'whiteAlpha.600'
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'colors',
  style: 'indicator-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'transparent'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'transparent'
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'colors',
  style: 'loading-indicator-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'primary.200'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'primary.400'
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'colors',
  style: 'text-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'blackAlpha.800'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'whiteAlpha.800'
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'colors',
  style: 'placeholder-text-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'blackAlpha.600'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'whiteAlpha.600'
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'colors',
  style: 'placeholder-selected-text-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'blackAlpha.800'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'whiteAlpha.800'
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'colors',
  style: 'icon-border-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'blackAlpha.200'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'whiteAlpha.300'
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'colors',
  style: 'menu-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'white'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'gray.900'
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'colors',
  style: 'option-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'transparent'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'transparent'
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'colors',
  style: 'option-hover-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'blackAlpha.100'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'whiteAlpha.100'
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'colors',
  style: 'option-active-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'gray.100'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'gray.700'
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'colors',
  style: 'option-focus-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'primary.100'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'primary.600'
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'colors',
  style: 'option-disabled-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'transparent'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'transparent'
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'colors',
  style: 'option-selected-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'primary.50'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'primary.500'
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'shadows',
  style: 'menu-shadow',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: '0 2px 6px 1px var(--chakra-colors-blackAlpha-200)'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: '0 3px 10px -2px var(--chakra-colors-blackAlpha-700)'
  }]
}, {
  componentName: 'change-chain-dropdown',
  category: 'shadows',
  style: 'shadow',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: '0 0 0 2px var(--chakra-colors-primary-200)'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: '0 0 0 2px var(--chakra-colors-primary-200)'
  }]
},
// simple-modal-head
{
  componentName: 'simple-modal-head',
  category: 'colors',
  style: 'text-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'gray.700'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'whiteAlpha.900'
  }]
}, {
  componentName: 'simple-modal-head',
  category: 'colors',
  style: 'button-icon-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'blackAlpha.600'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'whiteAlpha.600'
  }]
},
// simple-display-wallet-list
{
  componentName: 'simple-display-wallet-list',
  category: 'colors',
  style: 'button-text-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'blackAlpha.800'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'whiteAlpha.800'
  }]
}, {
  componentName: 'simple-display-wallet-list',
  category: 'colors',
  style: 'button-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'gray.100'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'blackAlpha.500'
  }]
}, {
  componentName: 'simple-display-wallet-list',
  category: 'colors',
  style: 'button-hover-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'gray.50'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'blackAlpha.600'
  }]
}, {
  componentName: 'simple-display-wallet-list',
  category: 'colors',
  style: 'icon-border-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'gray.100'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'gray.800'
  }]
}, {
  componentName: 'simple-display-wallet-list',
  category: 'colors',
  style: 'hover-icon-border-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'gray.50'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'gray.900'
  }]
}, {
  componentName: 'simple-display-wallet-list',
  category: 'colors',
  style: 'shadow-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'linear-gradient(0deg, rgba(255,255,255,1) 6%, rgba(255,255,255,0.95) 16%, rgba(255,255,255,0.85) 24%, rgba(255,255,255,0.75) 32%, rgba(255,255,255,0.65) 48%, rgba(255,255,255,0.4) 65%, rgba(255,255,255,0.2) 80%, rgba(255,255,255,0.1) 95%)'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'linear-gradient(0deg, rgba(45,55,72,1) 6%, rgba(45,55,72,0.95) 16%, rgba(45,55,72,0.85) 36%, rgba(45,55,72,0.75) 45%, rgba(45,55,72,0.65) 55%, rgba(45,55,72,0.4) 70%, rgba(45,55,72,0.2) 80%, rgba(45,55,72,0.1) 95%)'
  }]
},
// qr-code
{
  componentName: 'qr-code',
  category: 'colors',
  style: 'border-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'blackAlpha.200'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'whiteAlpha.200'
  }]
}, {
  componentName: 'qr-code',
  category: 'colors',
  style: 'qr-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'var(--chakra-colors-white)'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'var(--chakra-colors-gray-800)'
  }]
}, {
  componentName: 'qr-code',
  category: 'colors',
  style: 'qr-blur-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'whiteAlpha.700'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'blackAlpha.400'
  }]
}, {
  componentName: 'qr-code',
  category: 'colors',
  style: 'qr-text-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'var(--chakra-colors-blackAlpha-900)'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'var(--chakra-colors-white)'
  }]
}, {
  componentName: 'qr-code',
  category: 'colors',
  style: 'qr-expired-text-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'orange.300'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'orange.200'
  }]
}, {
  componentName: 'qr-code',
  category: 'colors',
  style: 'qr-error-text-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'red.500'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'red.400'
  }]
}, {
  componentName: 'qr-code',
  category: 'shadows',
  style: 'shadow',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: '0 2px 5px -1px var(--chakra-colors-blackAlpha-300)'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: '0 2px 5px -1px var(--chakra-colors-blackAlpha-900)'
  }]
}, {
  componentName: 'qr-code',
  category: 'shadows',
  style: 'button-shadow',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: '0 1px 4px var(--chakra-colors-blackAlpha-600), 0 5px 12px var(--chakra-colors-blackAlpha-400), 0 0 25px 6px var(--chakra-colors-whiteAlpha-600)'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: '0 1px 4px var(--chakra-colors-blackAlpha-900), 0 5px 12px var(--chakra-colors-blackAlpha-800), 0 0 25px 6px var(--chakra-colors-whiteAlpha-400)'
  }]
}, {
  componentName: 'qr-code',
  category: 'colors',
  style: 'shadow-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'linear-gradient(0deg, rgba(255,255,255,1) 6%, rgba(255,255,255,0.95) 16%, rgba(255,255,255,0.85) 24%, rgba(255,255,255,0.75) 32%, rgba(255,255,255,0.65) 48%, rgba(255,255,255,0.4) 65%, rgba(255,255,255,0.2) 80%, rgba(255,255,255,0.1) 95%)'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'linear-gradient(0deg, rgba(45,55,72,1) 6%, rgba(45,55,72,0.95) 16%, rgba(45,55,72,0.85) 36%, rgba(45,55,72,0.75) 45%, rgba(45,55,72,0.65) 55%, rgba(45,55,72,0.4) 70%, rgba(45,55,72,0.2) 80%, rgba(45,55,72,0.1) 95%)'
  }]
},
// display-modal-content
{
  componentName: 'simple-display-modal-content',
  category: 'colors',
  style: 'loading-border-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'purple.300'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'purple.400'
  }]
}, {
  componentName: 'simple-display-modal-content',
  category: 'colors',
  style: 'warning-border-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'orange.300'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'orange.400'
  }]
}, {
  componentName: 'simple-display-modal-content',
  category: 'colors',
  style: 'error-border-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'red.400'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'red.500'
  }]
}, {
  componentName: 'simple-display-modal-content',
  category: 'colors',
  style: 'warning-header-text-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'orange.300'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'orange.400'
  }]
}, {
  componentName: 'simple-display-modal-content',
  category: 'colors',
  style: 'error-header-text-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'red.400'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'red.500'
  }]
}, {
  componentName: 'simple-display-modal-content',
  category: 'colors',
  style: 'shadow-background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'linear-gradient(0deg, rgba(255,255,255,1) 6%, rgba(255,255,255,0.95) 16%, rgba(255,255,255,0.85) 24%, rgba(255,255,255,0.75) 32%, rgba(255,255,255,0.65) 48%, rgba(255,255,255,0.4) 65%, rgba(255,255,255,0.2) 80%, rgba(255,255,255,0.1) 95%)'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'linear-gradient(0deg, rgba(45,55,72,1) 6%, rgba(45,55,72,0.95) 16%, rgba(45,55,72,0.85) 36%, rgba(45,55,72,0.75) 45%, rgba(45,55,72,0.65) 55%, rgba(45,55,72,0.4) 70%, rgba(45,55,72,0.2) 80%, rgba(45,55,72,0.1) 95%)'
  }]
},
// simple-modal
{
  componentName: 'simple-modal',
  category: 'colors',
  style: 'background-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'white'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'gray.700'
  }]
}, {
  componentName: 'simple-modal',
  category: 'colors',
  style: 'text-color',
  theme: [{
    themeName: _types.Themes.Light,
    themeValue: 'gray.700'
  }, {
    themeName: _types.Themes.Dark,
    themeValue: 'whiteAlpha.900'
  }]
}];
exports.styleData = styleData;
var semanticTokens = {
  semanticTokens: handleData(styleData)
};
exports.semanticTokens = semanticTokens;